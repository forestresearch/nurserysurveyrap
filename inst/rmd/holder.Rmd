...
Data for the planting year October `r (ref_year - 2)` to September `r (ref_year -` are published for the first time in this release. Data for earlier years are unchanged from those previously released. For further details, see [Forest Research's revisions policy](https://www.forestresearch.gov.uk/tools-and-resources/statistics/about-our-statistics/code-of-practice/quality-of-official-statistics/).
...
-   A total of `r frpubutils::round_fmt_rmd(all_scotland_im_n, 1)` million plants (`r paste0(all_scotland_im_pct, " %")`) derived from improved nursery stock were sold to Scotland in `r latest_year`.

-   Of all sales in Great Britain in `r latest_year`, `r frpubutils::round_fmt_rmd(all_gb_im_n, 1)` million plants (`r paste0(all_gb_im_pct, " %")`) were derived from improved nursery stock, `r perc_chg_text(all_gb_diff)` of `r paste0(all_gb_pct, " %")` (or `r frpubutils::round_fmt_rmd(all_gb_diff, 1)` million plants) compared with the previous year.

\newpage

# Sales of nursery stock to Scotland

An estimated total of `r frpubutils::round_fmt_rmd(all_scotland_n, 1)` million Sitka spruce and Scots pine plants were sold to Scotland in `r latest_year` (Table 1), comprising `r frpubutils::round_fmt_rmd(scotland_sitka_n, 1)` million Sitka spruce plants and `r frpubutils::round_fmt_rmd(scotland_scots_n, 1)` million Scots pine plants. This represents `r frpubutils::perc_chg_txt(Scotland_sitka_pct * 100)` in sales of Sitka spruce (`r paste0(scotland_sitka_pct, " %")`) and `r frpubutils::perc_chg_txt(Scotland_scots_pct * 100)` in sales of Scots pine (`r paste0(scotland_scots_pct, " %")`) from the previous year.

A total of `r frpubutils::round_fmt_rmd(all_scotland_im_n, 1)` million plants (`r paste0(all_scotland_im_pct, " %")`) derived from improved nursery stock were sold to Scotland in `r latest_year`, `r frpubutils::perc_chg_text(all_scotland_im_ac)` of `r paste0(all_scotland_im_ac,  " %")` compared with `r previous_year`.
...
An estimated total of `r frpubutils::round_fmt_rmd(all_gb_n, 1)` million Sitka spruce and Scots pine plants were sold to Scotland in `r planting_year(current_year)` (Table 1), comprising `r frpubutils::round_fmt_rmd(gb_sitka_n, 1)` million Sitka spruce plants and `r frpubutils::round_fmt_rmd(gb_scots_n, 1)` million Scots pine plants. This represents `r frpubutils::perc_chg_txt((gb_sitka_pct * 100))` in sales of Sitka spruce (`r percent(gb_sitka_pct)`) and `r frpubutils::perc_chg_txt((gb_scots_pct * 100))` in sales of Scots pine (`r percent(gb_scots_pct)`) from the previous year.

A total of `r frpubutils::round_fmt_rmd(all_gb_im_n, 1)` million plants (`r percent(all_gb_im_pct)`) derived from improved nursery stock were sold to Scotland in `r planting_year(current_year)`, `r frpubutils::perc_chg_txt(all_gb_im_ac)` of `r percent(all_gb_im_ac)` compared with `r planting_year(prev_year)`.
