...
#afcharts::use_afcharts() # Error in `afcharts::use_afcharts()`: ! object 'af_colour_values' not found
```
...
-   A total of `r round_fmt_rmd(all_scotland_im_n, 1)` million plants (`r percent(all_scotland_im_pct)`) derived from improved nursery stock were sold to Scotland in `r planting_year(ref_year)`.

-   Of all sales in Great Britain in `r planting_year(ref_year)`, `r round_fmt_rmd(all_gb_im_n, 1)` million plants (`r percent(all_gb_im_pct)`) were derived from improved nursery stock, `r perc_chg_text(all_gb_diff)` of `r percent(abs(all_gb_pct))` (or `r round_fmt_rmd(abs(all_gb_diff), 1)` million plants) compared with the previous year.

# Sales of nursery stock to Scotland

An estimated total of `r round_fmt_rmd(all_scotland_n, 1)` million Sitka spruce and Scots pine plants were sold to Scotland in `r planting_year(ref_year)` (Table 1), comprising `r round_fmt_rmd(scotland_sitka_n, 1)` million Sitka spruce plants and `r round_fmt_rmd(scotland_scots_n, 1)` million Scots pine plants. This represents `r get_direction((scotland_sitka_pct * 100))` in sales of Sitka spruce (`r percent(scotland_sitka_pct)`) and `r get_direction((scotland_scots_pct * 100))` in sales of Scots pine (`r percent(scotland_scots_pct)`) from the previous year.

A total of `r round_fmt_rmd(all_scotland_im_n, 1)` million plants (`r percent(all_scotland_im_pct)`) derived from improved nursery stock were sold to Scotland in `r planting_year(current_year)`, `r get_perc_chg_text(all_scotland_im_ac)` of `r percent(all_scotland_im_ac)` compared with `r planting_year(ref_year)`.
...
rbind(table1, table1_sitka_tot, table1_tot) %>%
  ...
  rename(`Total stock sold in 2022/23 (million plants)` = 2,
         `Improved stock sold in 2022/23 (million plants)` = 3,
...
#### Figure 1: Sales of improved nursery stock, Scotland, `r planting_year(first_year)` to `r planting_year(current_year)`
...
#### Table 2: Sales of improved nursery stock, Scotland, `r planting_year(first_year)` to `r planting_year(current_year)`
...
An estimated total of `r round_fmt_rmd(all_gb_n, 1)` million Sitka spruce and Scots pine plants were sold to Scotland in `r planting_year(current_year)` (Table 1), comprising `r round_fmt_rmd(gb_sitka_n, 1)` million Sitka spruce plants and `r round_fmt_rmd(gb_scots_n, 1)` million Scots pine plants. This represents `r get_direction((gb_sitka_pct * 100))` in sales of Sitka spruce (`r percent(gb_sitka_pct)`) and `r get_direction((gb_scots_pct * 100))` in sales of Scots pine (`r percent(gb_scots_pct)`) from the previous year.

A total of `r round_fmt_rmd(all_gb_im_n, 1)` million plants (`r percent(all_gb_im_pct)`) derived from improved nursery stock were sold to Scotland in `r planting_year(current_year)`, `r get_direction(all_gb_im_ac)` of `r percent(all_gb_im_ac)` compared with `r planting_year(prev_year)`.



#### Table 3: Summary table, Great Britain, `r planting_year(current_year)`
...
#### Figure 2: Sales of improved nursery stock, Great Britain, `r planting_year(first_year)` to `r planting_year(current_year)`
...
Figure 2 shows the volume/quantity improved stock sold in Great Britain, from  `r planting_year(first_year)` to `r planting_year(current_year)`
...
#### Table 4: Sales of improved nursery stock, Great Britain, `r planting_year(first_year)` to `r planting_year(current_year)`
